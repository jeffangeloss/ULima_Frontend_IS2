...
| **Chat de sección** | [`.../chat.spec.md`](specs/features/chat/chat.spec.md) · 653 líneas · RF-CHAT-1 a RF-CHAT-13 | HU23 | `chat_page`, `chats_inbox_page`, botón de la ficha, tarjeta de Secciones | `ChatRepository`, `NetworkingService` | `HU23_jeff` (167, 9 archivos) | Fase 1 en el código y sin cerrar. Faltan el ajuste del footer del delegado (RF-CHAT-5), la revisión manual que pide «Verificación» en un iPhone SE, en claro y en oscuro, del footer del delegado, de la bandeja y de la conversación, la aprobación de los ajustes de app-shell, schedule, course-detail y time-blocks (RF-BLQ-8) y la confirmación del dueño del texto que la spec suma después de su aprobación. «Verificación» pide además `dart format` en `horario.dart` y en las dos pruebas de HU35, que no lo pasan desde antes de la fase, y el dueño elige entre un commit solo de formato y una excepción escrita en ella |
