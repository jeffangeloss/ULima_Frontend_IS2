...
| `ChatbotPage` | [`lib/pages/chatbot/chatbot_page.dart:13`](lib/pages/chatbot/chatbot_page.dart) | Alumno | ULimaBot: asistente académico con historial de conversaciones. Un mensaje que es solo un 67 recibe «SIX SEVEN!!!» en local e inclina la pantalla unos 2 s ([`specs/features/six-seven`](specs/features/six-seven/six-seven.spec.md)) | `ChatbotService`, `NotasService` | — |
...
| `ChatPage` | [`lib/pages/chat/chat_page.dart:17`](lib/pages/chat/chat_page.dart) | Alumno y docente | Chat de sección sobre Firebase RTDB, con el círculo del curso y «Sección N» en el `AppBar`. El alumno lo abre desde la bandeja `ChatsInboxPage` de la pestaña Chats o desde la ficha del curso. Un 67 que llega en vivo inclina la pantalla unos 2 s y muestra el rótulo «SIX SEVEN!!!» ([`specs/features/six-seven`](specs/features/six-seven/six-seven.spec.md)) | `ChatRepository`, `NetworkingService` | — |
...
| [`test/six_seven/`](test/six_seven) | Truco del 67 (HU-67-01 y HU-67-02) | jeff | 5 | 58 | Unitaria + Widget | `esSeisSiete` con cada caso de RF-67-1 y la curva del tambaleo; `TambaleoSeisSiete` con el giro de ±3° en 2000 ms, uno a la vez, movimiento reducido, teclado y rótulo; `DetectorSeisSiete` con los ids vistos; `ChatPage` con listas en vivo y el stream creado una vez; `ChatbotController` y `ChatbotPage` con un `ChatbotService` falso. Los casos con parámetros suben a 99 al correr |
