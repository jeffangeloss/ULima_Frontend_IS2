...
| **Superficie** | **28 pantallas** · 25 controllers · **29 services** · 21 modelos · 18 componentes |
| **Código** | 30 217 líneas Dart en 151 archivos bajo `lib/` |
| **Verificación** | **46 suites** · 6 456 líneas de prueba · `flutter analyze` como puerta obligatoria |
...
Este repositorio es **únicamente la app Flutter**: paquete Dart `ulima_plus` versión `1.0.0+1` ([`pubspec.yaml`](pubspec.yaml)), **30 217 líneas de Dart repartidas en 151 archivos** bajo [`lib/`](lib), **15 specs** en [`specs/features/`](specs/features) y **46 suites de prueba** (6 456 líneas) en [`test/`](test). Son 28 pantallas, 25 controllers GetX, 29 servicios, 21 modelos y 18 componentes. Las plataformas que realmente arrancan son **Android, iOS y Web**: `Firebase.initializeApp` corre siempre en el arranque ([`lib/main.dart:53`](lib/main.dart)) y `DefaultFirebaseOptions.currentPlatform` lanza `UnsupportedError` en macOS, Windows y Linux ([`lib/firebase_options.dart:27-45`](lib/firebase_options.dart)). Los directorios de esas tres plataformas existen en el repo, pero son andamiaje generado por `flutter create` que nadie compila.
...
│
├── test/                       (46)  # 6 456 líneas. 18 carpetas: test/HU##_<autor> por historia,
...
> contrato; el trabajo vive en `test/HU_asistencia/`, sin commitear.
...
| `RF-APP-33` | Mostrar el porcentaje de asistencia solo si el backend lo midió; con `asistenciaDisponible == false` la UI dice **sin datos**, nunca un porcentaje | [`lib/pages/descripcion_cursos/descrip_cursos.dart`](lib/pages/descripcion_cursos/descrip_cursos.dart) | `SeccionService` | **En curso, sin commitear** |
...
flowchart TD
  ...
  CODE --> TEST["46 archivos de prueba, 44 de ellos en test slash HU"]
...
| **HU19** | Malla móvil: filtros, progreso real y simulación | Alumno | `malla_list_page.dart`, `lib/domain/malla/**` | jeff | 55 | Implementado |
...
| **HU31** | Cargar el ciclo desde el portal miUlima | Alumno | `lib/pages/portal_sync/**` | jeff | 43 | Implementado; verificación E2E manual pendiente |
| **HU_asistencia** | Asistencia real por sección y estado `sin_datos` | Alumno / Docente | `descrip_cursos.dart`, `at_risk_students_*` | — | 16 | **En curso y sin commitear.** El frontend va por delante del backend: `sin_datos` aún no existe en `src/` |
...
| **Pruebas** | `test/HU19_jeff/malla_logic_test.dart` (31 casos), `malla_desbloqueo_cajablanca_test.dart`, `malla_progreso_real_test.dart`, `course_node_fromjson_cajanegra_test.dart`, `malla_list_page_test.dart`, `course_detail_sheet_test.dart` |
...
**46 archivos de prueba, 6 456 líneas, 336 casos declarados** — 287 `test(` y 49 `testWidgets(`.
...
La proporción es **6 456 líneas de prueba contra 30 217 de `lib/`**: aproximadamente una línea
...
- **Códigos de alumno con pinta de reales en los títulos de dos casos** de
  `test/HU19_jeff/malla_progreso_real_test.dart`. El repo es público. La práctica correcta ya
  existe en el árbol —`test/HU_asistencia/at_risk_student_sin_datos_test.dart` usa un código con
  el comentario `// sintetico: el repo es publico`— y falta homogeneizarla.
...
>
> **Pendiente de limpiar:** cinco archivos versionados usan códigos de alumno que aparentan ser
> reales, y no solo en pruebas: `test/HU19_jeff/malla_progreso_real_test.dart:53,66`,
> `test/HU01_jeff/login_relogin_regression_test.dart:81,83`,
