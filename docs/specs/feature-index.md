...
| 0 | Application Shell | `specs/features/app-shell/app-shell.spec.md` | Infra | BR-SHELL-F-00 a BR-SHELL-F-03: encabezado global, orientación del shell y pestañas del footer | `lib/main.dart`, `lib/components/header/app_header.dart`, `lib/pages/home/home_page.dart`, `lib/pages/home/home_shell_config.dart` | Activo; BR-SHELL-F-02 y BR-SHELL-F-03 (chats de curso) ya están en el código y siguen pendientes de aprobación |
...
| 5 | Schedule | `specs/features/schedule/schedule.spec.md` | US09 | R19 | `lib/pages/horario` | Spec existente; el ajuste de los chats de curso (Horario solo calendario) ya está en el código y sigue pendiente de aprobación |
| 6 | Course Detail | `specs/features/course-detail/course-detail.spec.md` | US13, US14, US17 | R20 | `lib/pages/descripcion_cursos` | Spec existente; el ajuste de los chats de curso (botón «Chat del curso») ya está en el código y sigue pendiente de aprobación |
...
| 18 | Bloques de horario propios | `specs/features/time-blocks/time-blocks.spec.md` | HU35 | RF-BLQ-1 a RF-BLQ-8: bloques propios del alumno en el horario (crear, editar, corregir un día suelto), aviso de cruce, reparto lado a lado con las clases, domingo en la vista semanal, horas por semana y la lista Mis bloques | `lib/pages/time_blocks/**`, `lib/services/time_blocks_service.dart`, `lib/models/time_block_model.dart`, `lib/pages/horario/horario.dart`, `lib/pages/horario/horario_controller.dart`, `lib/pages/horario/horario_layout.dart` | Implementado — pendiente de probar contra el backend desplegado; el ajuste de RF-BLQ-8 por los chats de curso (el botón «Mis bloques» sin la condición de la lista de chats) ya está en el código y sigue pendiente de aprobación |
| 19 | Chat de sección | `specs/features/chat/chat.spec.md` | HU23 | RF-CHAT-1 a RF-CHAT-13: chat en vivo por sección (token, mensajes, texto y carnet, moderación del profesor), pestaña Chats del alumno con su bandeja, botón «Chat del curso» en la ficha, conversación con la identidad de la app y acceso del docente desde Secciones | `lib/pages/chat/**`, `lib/services/chat_repository.dart`, `lib/models/message.dart`, `lib/pages/home/home_shell_config.dart`, `lib/pages/home/home_page.dart`, `lib/components/header/app_header.dart`, `lib/pages/horario/horario.dart`, `lib/pages/horario/horario_controller.dart`, `lib/pages/horario/horario_list_view.dart` (borrado en la fase 1), `lib/pages/descripcion_cursos/descrip_cursos.dart`, `lib/pages/teacher/teacher_sections_page.dart`, `lib/configs/themes.dart` | Aprobada (fase 1), con el código escrito y la fase sin cerrar. Faltan el ajuste del footer del delegado, que RF-CHAT-5 deja al dueño, y la revisión manual que pide «Verificación» en un iPhone SE, en claro y en oscuro, del footer del delegado, de la bandeja y de la conversación. «Verificación» pide además `dart format` en `horario.dart` y en las dos pruebas de HU35, que no lo pasan desde antes de la fase, y el dueño elige entre un commit solo de formato y una excepción escrita en ella. Los ajustes de app-shell (BR-SHELL-F-02 y BR-SHELL-F-03), schedule, course-detail y time-blocks (RF-BLQ-8) siguen pendientes de aprobación, y el texto que la spec suma después de su aprobación espera la confirmación del dueño |
