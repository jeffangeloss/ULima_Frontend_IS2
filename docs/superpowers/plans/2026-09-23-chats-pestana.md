...
**Arquitectura:** piezas puras nuevas (iniciales del curso, agrupación de mensajes, día y hora en Lima) y dos tokens de tema; `ChatPage` se rediseña sobre esas piezas; una página de bandeja nueva lee de `HorarioController`, que ya carga las secciones; Horario pierde su vista de lista y el header su toggle.

**Stack:** Flutter 3.x + GetX + Firebase Realtime Database (sin cambios) + `lucide_icons` (ya en el proyecto).

**Spec:** `specs/features/chat/chat.spec.md` (RF-CHAT-1 a RF-CHAT-13, aprobada el 2026-09-23), más los ajustes de `specs/features/app-shell/app-shell.spec.md` (BR-SHELL-F-02 y F-03), `specs/features/schedule/schedule.spec.md` («Solo calendario») y `specs/features/course-detail/course-detail.spec.md`. **La spec es la fuente de los valores exactos** (tamaños, colores, textos, cifras de contraste, reglas); este plan dice en qué orden, en qué archivos y con qué pruebas. Si el plan y la spec difieren, manda la spec.
...
|---|---|---|
| `lib/configs/themes.dart` | modificar | tokens `chatOwnBubbleBg(Brightness)` y `errorBg(Brightness)` |
| `lib/pages/chat/chat_timeline.dart` | crear | funciones puras: iniciales, color de iniciales, día y hora en Lima, etiqueta de día, inicio de grupo, nombre |
| `lib/components/course_avatar.dart` | crear | `CourseAvatar`: círculo del color del curso con sus iniciales (bandeja y AppBar) |
...
**Archivos:** modificar `lib/configs/themes.dart`; crear `lib/pages/chat/chat_timeline.dart`, `lib/components/course_avatar.dart`; pruebas `test/HU23_jeff/chat_linea_tiempo_test.dart` y `test/HU23_jeff/chat_identidad_test.dart`.
...
// lib/pages/chat/chat_timeline.dart
...
// lib/components/course_avatar.dart
class CourseAvatar extends StatelessWidget {
  const CourseAvatar({super.key, required this.nombre, required this.color, this.size = 42});
...
- [ ] **Paso 2: pruebas que fallan.** Escribe las dos pruebas con estos casos, como mínimo:
  ...
  - `CourseAvatar`: pinta las iniciales y el color; con nombre sin letras no pinta texto.
...
**Consume:** todo lo que produce la Tarea 1.
...
- [ ] **Paso 1: pruebas que fallan** (con el `ChatRepositoryContract` falso que ya usa `chat_page_test.dart`):
  - AppBar con `CourseAvatar`, título = curso, subtítulo «Sección 801» y «Sin sección» con código vacío; sin «Chat grupal».
...
**Consume:** `CourseAvatar` (Tarea 1) y `ChatPage(sectionCode:, courseColor:)` (Tarea 2).
...
- [ ] **Paso 1: pruebas que fallan:**
  ...
  - Contraste de «Chat del curso», «Chat» y sus íconos en los dos temas con `contrasteWcag`.
