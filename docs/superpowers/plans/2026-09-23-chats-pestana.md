...
**Spec:** `specs/features/chat/chat.spec.md` (RF-CHAT-1 a RF-CHAT-13, aprobada el 2026-09-23), más los ajustes de `specs/features/app-shell/app-shell.spec.md` (BR-SHELL-F-02 y F-03), `specs/features/schedule/schedule.spec.md` («Solo calendario») y `specs/features/course-detail/course-detail.spec.md`. Estos tres ajustes siguen pendientes de la aprobación del dueño. **La spec es la fuente de los valores exactos** (tamaños, colores, textos, cifras de contraste, reglas); este plan dice en qué orden, en qué archivos y con qué pruebas. Si el plan y la spec difieren, manda la spec.
...
- [ ] **Paso 1:** en `specs/features/chat/chat.spec.md`, cada `[@test]` apunta a un archivo que existe y que tiene al menos un caso del requisito que lo enlaza (por ejemplo, RF-CHAT-10 en `chat_identidad_test.dart` y en `chat_page_test.dart`). El estado de la spec suma «e implementada» con la fecha, y la fila 19 de `docs/specs/feature-index.md` pasa a implementada, solo cuando RF-CHAT-5 da la fase 1 por terminada. Esta condición pide que las seis etiquetas del footer del delegado se lean completas en 375 pt con cualquiera activa, y en el iPhone SE la comprueba la revisión manual de «Verificación», porque `chats_pestana_test` mide con Roboto. Mientras la condición no se cumpla, el estado queda como aprobado y la fila 19 dice que la fase 1 tiene el código escrito y sigue sin cerrar. El ajuste del footer (la etiqueta activa a 13 px, todas a 12 px o «Deleg.») y el cierre formal de la fase 1 quedan para el dueño. Las notas de app-shell (BR-SHELL-F-02 y BR-SHELL-F-03), schedule y course-detail conservan «Pendiente de la aprobación del dueño» aunque la fase 1 ya las implemente, porque solo el dueño las aprueba. Este paso ya no pide «e implementada» sin la condición de RF-CHAT-5, porque la spec manda sobre el plan.
